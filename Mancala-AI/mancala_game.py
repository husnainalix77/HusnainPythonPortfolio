...
class Board:
    """Represents the Mancala board state and all pit/store operations."""

    # Constants — never change during the game
    P1_STORE = 6
    P2_STORE = 13
    ...
    def __init__(self) -> None:
        """Initialize board with 4 stones in each pit and empty stores."""
        self.board = [4, 4, 4, 4, 4, 4, 0, 4, 4, 4, 4, 4, 4, 0]
    
    def __str__(self) -> str:
        """Return a colored, human-readable board layout."""
        # AI pits displayed in reverse (right to left on top row)
...
"""
    ...
    def p1_score(self) -> int:
        """Return Player 1's current store count."""
        return self.board[Board.P1_STORE]

    @property
    def p2_score(self) -> int:
        """Return Player 2's current store count."""
        return self.board[Board.P2_STORE]    
    
    def get_pit(self, index: int) -> int:
        """Return the stone count at a given pit index."""
        return self.board[index]
    
    def set_pit(self, index: int, value: int) -> None:
        """Set the stone count at a given pit index."""
        self.board[index] = value  
    
    def is_pit_empty(self, index: int) -> bool:
        """Check whether a pit has no stones."""
        return self.board[index] == 0    
    
    def get_opposite_pit(self, index: int) -> int:
        """Return the index of the pit directly opposite (used for captures)."""
        return 12 - index  

    def is_side_empty(self) -> int | bool:
        """Check if either player's side is fully empty; returns player number or False."""
    ...
    def sweep_remaining(self, player_num: int) -> None:
        """Collect all remaining stones on a player's side into their store (end-of-game)."""
        if player_num == 1:
            pits  = Board.P1_PITS
    ...
    def fill_pit(self, player_num: int, pit_num: int) -> int | None:
        """
        Sow stones from the chosen pit counter-clockwise, skipping the opponent's store.
        Returns the index of the last pit where a stone was placed, or None if pit is empty.
        """
        # Map pit number to board index based on player
        if player_num == 1:
            current = pit_num
        else:
            current = pit_num + 7
    ...
    def check_capture(self, current_pit: int, player_num: int) -> bool:
        """
        Capture opponent's stones if last stone lands in an empty pit on the player's side
        with a non-empty opposite pit. Returns True if a capture occurred.
        """
        ...
        if self.board[current_pit] == 1 and current_pit != store and current_pit in valid_pits:
            ...
            if self.board[opp_pit] == 0:
                return False
            # Move captured stones to player's store
    ...
    def is_winner(self) -> bool:
        """Print the winner based on final store counts and return True."""
...
class Player:
    """Represents a human player with associated pits and store."""
    
    def __init__(self, name: str, player_num: int) -> None:
        """Initialize player with name, number, and board index assignments."""
        self.name = name
    ...
    @staticmethod        
    def show_menu() -> int:
        """Display the first-turn selection menu and return the starting player number."""
...
class AIPlayer(Player):
    """AI player using Minimax with Alpha-Beta pruning and adaptive search depth."""
    
    def __init__(self, name: str, player_num: int) -> None:
        """Initialize AI player (inherits from Player)."""
        super().__init__(name, player_num)
    
    def getAdaptiveDepth(self, board_obj) -> int:
        """Return search depth based on total stones remaining (deeper near endgame)."""
        ...
        if total_stones > 30:
            ...
            return 7      # Late game: deep search
        
    def get_valid_pits(self, board_obj, player_num: int) -> list:
        """Return list of non-empty pit indices for the given player."""
    ...
    def simulate_move(self, board_obj, pit: int, player_num: int):
        """
        Simulate a move on a copy of the board without modifying the original.
        Returns (new_state, last_pit_index).
        """
        saved = board_obj.get_state()
        if player_num == 1:
            pit_num = pit
        else:
            pit_num = pit - 7       # Convert board index to pit number for AI
        last = board_obj.fill_pit(player_num, pit_num)
        new_state = board_obj.get_state()
        board_obj.set_state(saved)  # Restore original state after simulation
        return new_state, last   

    def _calculate_landing(self, pit: int, stones: int, player_num: int) -> int:
        """Calculate the landing index after distributing stones, respecting skip rules."""
        current = pit
    ...
    def evaluate_board(self, board_obj) -> float:
        ...
        Considers: store difference, bonus turn opportunities, capture threats, stone distribution.
        """
        # Terminal state check — sweep and score
        ...
        if empty_side:
            ...
            if diff > 0:  return 1000 # AI Wins
            if diff < 0:  return -1000 # Human Wins
            return 0 # Draw

        # Base score: AI store minus human store
        ...
        # Bonus: reward AI pits that can land in the store (extra turn)
        for p in ai_pits:
            if board_obj.board[p] == Board.P2_STORE - p:
                score += 3
        # Penalty: opponent pits that can land in their store
        for p in human_pits:
            if board_obj.board[p] == Board.P1_STORE - p:
                score -= 3  
        
        # Bonus: AI capture opportunities (land on empty AI pit with stones opposite)
        for p in ai_pits:
        ...
        # Penalty: human capture opportunities
        for p in human_pits:
            stones  = board_obj.board[p]
        ...
        # Small weight for stone distribution advantage        
    ...
    def minimax(self, board_obj, depth: int, is_maximizing: bool, 
        ...
        Maximizes AI score and minimizes human score recursively up to given depth.
        """
        # Base Case
        ...
        if is_maximizing:
            ...
            for pit in valid_pits:
                ...
                # Extra turn: AI lands in own store — stays maximizing
        ...
        # Human
            ...
            for pit in valid_pits:
                ...
                board_obj.set_state(new_state)
                board_obj.check_capture(last, current_player)
                # Extra turn: human lands in own store — stays minimizing
    ...
    def get_best_pit(self, board_obj, player_num: int) -> int | None:
        """Evaluate all valid moves and return the pit index with the highest Minimax score."""
        depth      = self.getAdaptiveDepth(board_obj) # Depth
        ...
        if not all_valid_pits: # Safety Check
            return None 
        ...
        for pit in all_valid_pits:
            ...
            # Extra turn case: AI stays as maximizer
            if last == Board.P2_STORE:
...
class Game:
    """Manages the overall game flow, turn switching, and player interaction."""

    def __init__(self, player1: Player, player2: AIPlayer) -> None:
        """Initialize the game with a fresh board and two players."""
        self.board = Board()
    ...
    def clear_screen() -> None:
        """Clear the terminal screen with a short delay for readability."""
    ...
    def valid_input(player_num: int) -> int:
        """Prompt the human player for a valid pit choice (1–6) with error handling."""
        while True:
            try:
    ...
    def start(self) -> None:
        """Show the menu, display the initial board, and begin the game loop."""
    ...
    def game_loop(self) -> None:
        """Main loop: alternate turns between human and AI until the game ends."""
        while True:
            if isinstance(self.player2, AIPlayer) and self.current_player_num == 2:
                # AI turn: compute best move and apply it
                ...
                # Human turn: get validated pit input
            ...
            # Check for bonus turn (last stone in own store)
            ...
            # Check end condition: one side is fully empty
